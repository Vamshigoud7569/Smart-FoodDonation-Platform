...
import {
  Heart, Bell, ShieldCheck, UserCircle,
  Clock, Save, LogOut, KeyRound, X, Check, AlertTriangle,
} from "lucide-react";
...
const EMPTY_PASSWORD_FIELDS: PasswordFields = {
  current: "",
  new: "",
  confirm: "",
};
...
const Settings: React.FC = () => {
  ...
  const [newPassword, setNewPassword] = useState<PasswordFields>(EMPTY_PASSWORD_FIELDS);
  const navigate = useNavigate();

  const updatePasswordField = (key: keyof PasswordFields, value: string) => {
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword.new === newPassword.current) {
      showToast("New password and current password should not match");
      return;
    }

    try {
      await foodDonationService.updatePassword({
        currentPassword: newPassword.current,
        newPassword: newPassword.new,
      });
      setShowPassword(false);
      setNewPassword(EMPTY_PASSWORD_FIELDS);
      showToast("Password updated");
    } catch (error: any) {
      showToast(error?.response?.data?.message || "Error updating password");
    }
  };
    ...
    <div className="ss-page">
      <div className="ss-container">
        ...
        {tab === "preferences" && (
          <section className="ss-card">
            <div className="ss-card-title">
              <div className="ss-icon-badge green"><Heart size={18} /></div>
              <h2>Donation preferences</h2>
            </div>

            <Row title="Accept donation requests" desc="Let recipients send you pickup requests">
              <Switch checked={draft.acceptRequests} onChange={() => update("acceptRequests", !draft.acceptRequests)} label="Accept donation requests" />
            </Row>
            <Row title="Available for donation" desc="Show up when nearby recipients are searching">
              <Switch checked={draft.availableForDonation} onChange={() => update("availableForDonation", !draft.availableForDonation)} label="Available for donation" />
            </Row>
            <Row title="Preferred donation time" desc="When you're usually free to hand off food" last>
              <div className="ss-time-select">
                <Clock size={15} />
        ...
        {tab === "notifications" && (
          <section className="ss-card">
            <div className="ss-card-title">
              <div className="ss-icon-badge orange"><Bell size={18} /></div>
              <h2>Notifications</h2>
            </div>
            ...
            <Row title="Pickup reminders" desc="A nudge before a scheduled pickup">
              <Switch checked={draft.pickupReminders} onChange={() => update("pickupReminders", !draft.pickupReminders)} label="Pickup reminders" />
            </Row>
            <Row title="Donation updates" desc="When a donation is accepted or completed" last>
              <Switch checked={draft.donationUpdates} onChange={() => update("donationUpdates", !draft.donationUpdates)} label="Donation updates" />
            </Row>
          </section>
        )}
        ...
        {tab === "privacy" && (
          <section className="ss-card">
            <div className="ss-card-title">
              <div className="ss-icon-badge green"><ShieldCheck size={18} /></div>
              <h2>Privacy</h2>
            </div>

            <Row title="Show contact number" desc="Visible to recipients once a donation is confirmed">
            ...
            <Row title="Share location with recipients" desc="Helps them find you for pickup" last>
              <Switch checked={draft.shareLocation} onChange={() => update("shareLocation", !draft.shareLocation)} label="Share location with recipients" />
            </Row>
          </section>
        )}
        ...
        {tab === "account" && (
          <section className="ss-card">
            <div className="ss-card-title">
              <div className="ss-icon-badge orange"><UserCircle size={18} /></div>
              <h2>Account</h2>
            </div>
            ...
            <div className="ss-account-row">
              ...
              <button className="ss-btn-secondary" onClick={() => setShowPassword(true)}>
            ...
            <div className="ss-account-row last">
              ...
              <button className="ss-btn-danger-ghost" onClick={() => setShowLogout(true)}>
        ...
        <div className="ss-modal-overlay" onClick={() => setShowPassword(false)}>
          <div className="ss-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ss-modal-actions">
              ...
              <button className="ss-modal-confirm green" onClick={handleChangePassword}>
                Update password
              </button>
            </div>
          </div>
        ...
        <div className="ss-modal-overlay" onClick={() => setShowLogout(false)}>
          <div className="ss-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ss-modal-actions">
              ...
              <button
                className="ss-modal-confirm orange"
...
const Row: React.FC<{ title: string; desc: string; children: React.ReactNode; last?: boolean }> = ({ title, desc, children, last }) => (
  <div className={`ss-row ${last ? "last" : ""}`}>
    <div>
      <h4>{title}</h4>
      <p>{desc}</p>
    </div>
