...
import {
  HeartHandshake, Bell, ShieldCheck, UserCircle,
  Save, LogOut, KeyRound, X, Check, AlertTriangle,
} from 'lucide-react';
...
const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: 'preferences',   label: 'Preferences',   icon: <HeartHandshake size={16} /> },
...
const RecipientSettings: React.FC = () => {
  ...
  const update = <K extends keyof Prefs>(key: K, value: Prefs[K]) => setDraft(p => ({ ...p, [key]: value }));
  ...
  const handleChangePassword = async () => {
    ...
    if (pw.new === pw.current) { showToast('New password must differ from current'); return; }
    try {
    ...
    <div className="ss-page">
      <div className="ss-container">
        ...
        {tab === 'preferences' && (
          <section className="ss-card">
            <div className="ss-card-title">
              <div className="ss-icon-badge green"><HeartHandshake size={18} /></div>
              <h2>Recipient preferences</h2>
            </div>
            <Row title="Accept donations" desc="Allow donors to send you food donations">
              <Switch checked={draft.acceptDonations} onChange={() => update('acceptDonations', !draft.acceptDonations)} label="Accept donations" />
            </Row>
            <Row title="Weekly digest" desc="A summary of nearby donations every Monday" last>
              <Switch checked={draft.weeklyDigest} onChange={() => update('weeklyDigest', !draft.weeklyDigest)} label="Weekly digest" />
            </Row>
          </section>
        )}
        ...
        {tab === 'notifications' && (
          <section className="ss-card">
            ...
            <Row title="Donation alerts" desc="Get notified when new donations are available" last>
              <Switch checked={draft.donationAlerts} onChange={() => update('donationAlerts', !draft.donationAlerts)} label="Donation alerts" />
            </Row>
          </section>
        )}
        ...
        {tab === 'privacy' && (
          <section className="ss-card">
            ...
            <Row title="Show contact number" desc="Visible to donors once a request is confirmed">
            ...
            <Row title="Share location with donors" desc="Helps donors find you for delivery" last>
              <Switch checked={draft.shareLocation} onChange={() => update('shareLocation', !draft.shareLocation)} label="Share location" />
            </Row>
          </section>
        )}
          ...
          <section className="ss-card">
            ...
            <div className="ss-field-grid">
              <div className="ss-field">
                ...
                <input type="text" value={draft.name} onChange={e => update('name', e.target.value)} placeholder="Your name" />
              </div>
              <div className="ss-field">
                <label>Phone number</label>
                <input type="text" value={draft.phone} onChange={e => update('phone', e.target.value)} placeholder="+91 98765 43210" />
              </div>
            </div>
            <div className="ss-account-row">
              <div><h4>Change password</h4><p>Update the password you use to sign in</p></div>
            ...
            <div className="ss-account-row last">
              <div><h4>Logout</h4><p>Sign out of your account on this device</p></div>
      ...
      <div className={`ss-save-bar ${dirty ? 'visible' : ''}`}>
        <div className="ss-save-bar-inner">
          ...
          <button className="ss-save-btn" onClick={() => { setSaved(draft); showToast('Settings saved'); }}>
      ...
      {toast && (
        <div className="ss-toast"><Check size={15} /><span>{toast}</span></div>
      )}
    </div>
  );
};
